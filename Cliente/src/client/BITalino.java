...
public class BITalino {
    ...
    private Frame[] decode(byte[] buffer) throws IOException, BITalinoException {
        /**
         * Unpack a raw byte stream into a frames vector. \param[in] buffer
         * Vector with the bytes read from the device. \return Vector of frames
    ...
    public Frame[] read(int nSamples) throws BITalinoException {
         ...
         * timeout occurs. \param[in] nSamples Number of frames that should be
         * read from the device. \return Vector of frames obtained from the
         * device. \remarks If a problem occurred, the size of the frames vector
         * is lower than the frames vector size. This method must be called only
         * during an acquisition. \exception BITalinoException
         * (BITalinoErrorTypes.LOST_COMMUNICATION)
         */
        try {
